...
- [ ] `src/keymap/action.rs`: `EditorAction` enum + `FromStr` / `Display`(action 名は SPEC-0004 の変換表を正とする)
...
- [ ] `src/keymap/context.rs`: `EditorContext`(SPEC-0002 のフィールド全部。reserved の `suggestVisible` / `quickOpenVisible` 含む)。`Default` は全 false + `editorFocus`/`textInputFocus` = true
- [ ] `src/keymap/predicate.rs`: `ContextPredicate`
...
- [ ] `src/keymap/key_parse.rs`: key 文字列 parser
...
- [ ] `src/keymap/binding.rs`: `Binding { keys: Vec<KeyEvent>, action: EditorAction, when: Option<ContextPredicate>, source: Source }`、`Source { Rescue, User, Imported, Default }`
- [ ] `src/keymap/resolver.rs`: `Resolver`
...
- [ ] `"ctrl+shift+j"` → Ctrl+Shift+`j`、`"cmd+s"` → Super+`s`、`"f1"` → F1、`"ctrl+space"` → Ctrl+Space
- [ ] `"ctrl+x ctrl+s"` → 2 chord の sequence
- [ ] `"CTRL+J"` が小文字と同じ結果になる
- [ ] `"ctrl+unknown"` / `"foo+j"` がエラーになる

predicate:

- [ ] `"editorFocus"` / `"!isReadonly"` / `"editorFocus && !isReadonly"` の parse と eval
- [ ] `"resourceLangId"` など未知 identifier が `UnknownContext` エラーになる
- [ ] `term_count` が項数を返す

resolver(source 優先度・限定性・定義順):

- [ ] 同じ key に Default と User がある → User が勝つ
- [ ] 同じ key に Imported と Rescue がある → Rescue が勝つ
- [ ] 同 source で `when` なし vs `editorFocus`(真)→ 限定的な方が勝つ
- [ ] 同 source・同 term 数 → 後に定義された方が勝つ
- [ ] `when` が偽の binding は候補にならない(`searchVisible` 偽のとき search overlay 用 binding が無効)
- [ ] context が変わると同じ key が別 action に解決される(`Ctrl+j`: editor では `cursor.down`、`searchVisible` では `search.next` のような 2 binding)

sequence:

- [ ] `"ctrl+x ctrl+s"` 登録時、`ctrl+x` 単発 → `Pending`(candidates に続きが入る)、続けて `ctrl+s` → `Matched`
- [ ] `ctrl+x` 単発 binding と `"ctrl+x ctrl+s"` が並ぶ → `ctrl+x` で `Pending { exact: Some(単発の action) }`
- [ ] 無関係 key → `NoMatch`
