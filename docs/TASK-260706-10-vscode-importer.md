...
- [ ] `src/keymap/vscode_when.rs`: VS Code `when` 式 → 内部 predicate 文字列への変換
...
- [ ] `src/keymap/vscode_import.rs`: importer 本体
...
- [ ] `src/keymap/report.rs`: `ImportReport`
...
- [ ] key 列の設定ファイル向け直列化: `format_key_for_config(&[KeyEvent]) -> String`(`"ctrl+shift+j"` / `"ctrl+k ctrl+s"` 形式。`parse_key_sequence` との round-trip をテストで保証)

### app 層

- [ ] `src/app/import_cli.rs`: `coda keymap import vscode <path> [--dry-run] [--replace] [--print-report]`(SPEC-0005)
...
- [ ] `src/app/config.rs`: 起動時に `generated/vscode-bindings.json` を `Source::Imported` として読み込む(`bindings.json` の user loader を流用し、source を Imported に差し替え。優先順位は resolver が処理する)。壊れていても起動は止めず警告
- [ ] `src/app/mod.rs`: `keymap import vscode` サブコマンドの routing 追加

## testcases

importer(table-driven。全分類を含む fixture JSON を使う):

- [ ] `cursorDown`(when: `editorFocus`)→ Imported、内部で `cursor.down` / `editorFocus`
- [ ] `cursorDownSelect` → `selection.down`
- [ ] `workbench.action.terminal.new` → Ignored(outside editor scope)
- [ ] `extension.foo` / `projectManager.list` → Ignored
- [ ] `editor.action.rename` → Unsupported command(feature not implemented)
- [ ] `-cursorDown` → Unsupported command(negative binding)
- [ ] when `resourceLangId == markdown` → Unsupported condition
- [ ] when `editorTextFocus && !editorReadonly` → Imported(`textInputFocus && !isReadonly` に変換)
- [ ] key `ctrl+[IntlBackslash]` 相当の parse 不能 key → Invalid key
- [ ] 同一 key・同一 when の 2 entry → 後勝ちで 1 binding + Conflict 1 件
- [ ] 全 entry 数 = 各分類の合計(黙って消えた entry がない)
- [ ] report の `render_text()` に SPEC-0004 の件数行と Examples 行が含まれる(golden 比較)
- [ ] `format_key_for_config` → `parse_key_sequence` の round-trip(`ctrl+shift+j` / `cmd+s` / `ctrl+k ctrl+s` / `f1` / `ctrl+space`)

app:

- [ ] generated/vscode-bindings.json を書いた後の config::load が Imported source の binding を返す
- [ ] `--dry-run` がファイルを書かない(unit で fs 副作用を検証)
- [ ] 品質: `cargo fmt --check` / `cargo clippy --all-targets -- -D warnings` / `cargo test` がすべて通る

手動(main agent が実施):

- [ ] 実際の VS Code keybindings.json サンプルで import → report 確認 → editor 起動して imported binding が効く
