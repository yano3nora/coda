# TASK-260706: 初回 dogfood フィードバック修正
...
[event loop editor task](TASK-260706-event-loop-editor.md)の editor を人間が実機(Ghostty)で試した結果、以下のフィードバックを得た。
...
- editor 本体のスクロール(viewport)は event loop editor 実装に含まれており、フィードバック 2 は palette リストの話と解釈した。もし editor 側のスクロール不具合だった場合は再報告を受けて別タスク化する
