...
- [ ] `src/app/document.rs`: 単一 buffer の状態を `Document` に集約する
...
- [ ] CLI: `coda a.txt b.txt` で全引数を buffer として開く(先頭を active に。同一 path の重複指定は 1 つに)
- [ ] dispatch 結線:
...
- [ ] context: `tabFocus` は今回も常に false のままでよい(tab への focus 移動は作らない。切替は action のみ)
- [ ] default bindings: `ctrl+tab` / `ctrl+shift+tab` → `buffer.next/previous`、`ctrl+w` と `cmd+w` → `buffer.close`(cmd+w は terminal に食われる環境では単に届かない)

## testcases

- [ ] Document 化後の既存全テストが通る(regression)
- [ ] 複数 path 指定で documents が引数順に並び、active = 0。重複 path は 1 つ
- [ ] `buffer.next/previous` の巡回(3 buffer で next×3 が一周)
- [ ] `buffer.close`: clean buffer は即 close、modified は 1 回目警告 → 2 回目 close
- [ ] 最後の buffer の close が quit 判定になる(modified なら警告経由)
- [ ] buffer ごとに undo / saved_snapshot / cursor が独立している(A で編集 → B へ切替 → A に戻ると状態が残っている)
- [ ] tab bar 描画: active の reverse、modified の `[+]`、幅超過の `…`(pure な描画関数として test)
- [ ] `app.quit` が「いずれかの buffer が modified」で警告する
- [ ] `cargo fmt --check` / `cargo clippy --all-targets -- -D warnings` / `cargo test` がすべて通る

手動(main agent PTY):

- [ ] 2 ファイルで起動 → ctrl+tab 切替 → 双方編集 → 個別 save → 内容が正しい
