...
- [x] `ui::Style` に `fg: Option<(u8, u8, u8)>` を追加する(bg は追加しない: MVP は fg のみで terminal の背景色を尊重する。ADR-0006)
- [x] `ColorMode { TrueColor, Ansi256, Mono }` を追加し、renderer が Style 直列化時に参照する
...
- [x] `src/highlight/cache.rs`: 行単位の incremental cache
...
- [x] `src/app/editor_view.rs`: 行描画時に span の色を `Style.fg` へ反映(selection の reverse が優先。Tab 展開部分は無色)
- [x] `src/app/config.rs`: `config.toml` 読込を追加
...
- [x] event loop / view の結線: ファイルの path から syntax を決め、編集のたびに cache 経由で再計算(cache が差分再計算を担う)

## testcases

- [x] RGB→256 変換: 純色・グレー・白黒の既知ベクタ
- [x] ColorMode 検出: COLORTERM=truecolor / TERM=xterm-256color / TERM=dumb の 3 系
- [x] renderer: fg 付き Style の run が TrueColor / Ansi256 / Mono で期待どおりの SGR になる
- [x] engine: `.rs` で syntax が見つかり、拡張子不明で None
- [x] cache: 初回計算 → 中間行を編集 → **編集行より前は再計算されない**(計算回数 or 状態比較で検証)+ 編集行以降が新しい span になる
- [x] cache: span が grapheme range で返る(日本語コメント行で桁ズレしない)
- [x] 2,000 bytes 超の行が無色 / 20,000 行超で全体 off
- [x] config.toml: theme=light / 未指定 / 壊れた TOML(警告 + default)
- [x] `cargo fmt --check` / `cargo clippy --all-targets -- -D warnings` / `cargo test` がすべて通る

手動(main agent PTY + 人間):

- [x] PTY: .rs ファイルを開いた出力に `38;2;`(truecolor SGR)が含まれる (6 色検出。base16-ocean.dark のパレットと一致)
- [ ] 人間: Ghostty で Rust / TOML ファイルが色付き表示され、編集してもハイライトが追従する。theme = "light" 切替

## notes

- レビュー指摘なし (2026-07-06 main agent 検証済み: 111 tests、core/keymap から highlight への参照ゼロを grep 確認、PTY で truecolor SGR 6 色検出)。実機の見た目と theme=light は人間確認待ち
